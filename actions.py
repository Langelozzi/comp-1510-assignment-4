...
def spider_web_blockade(character: dict) -> None:
    ...
    print_in_color(
        "You might be able to use one of your abilities to clear the webs!\n", "cyan")
    ...
    while ability_used[0] != "Fireball":
        print_in_color(
            f"I don't think {ability_used[0]} will work here, try a different one.", "red")
        ability_used = select_ability(character)
...
def generate_enemy_battle(enemy_data: dict):
    def enemy_battle(character: dict):
        print_in_color(
            f"Out of the corner of your eye you see a {enemy_data['name']} appear!", "cyan")
...
def generate_riddle(riddle_data: dict):
    def riddle_success(character: dict) -> None:
        ...
        success_options = list(
            enumerate(["Try my luck at a new ability", "Refill HP to max"], start=1))
        ...
        if int(user_choice) == 1:
            ...
            if (
                ...
                print(f"\nYou got lucky! I am feeling generous and will grant you a new ability. You can now use "
                      f"{new_ability}")
                print_in_color(
                    f"[{character['name']} | abilities: +'{new_ability}']", "yellow")
            else:
                print(
            ...
            print_in_color(
                f"[{character['name']} | xp: +{difference}]", "yellow")
    ...
    def riddle(character: dict) -> None:
        ...
        user_answer_string = [option for number,
                              option in options if number == int(user_answer)][0]
        if user_answer_string == riddle_data["answer"]:
            riddle_success(character)
        else:
            print_in_color(
                "That is not correct, and for that you must be punished!", "red")
